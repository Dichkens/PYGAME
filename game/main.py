...
startArea = pygame.Rect(mainScreen.get_width() / 2 - 100, mainScreen.get_height() / 2 + 50, mainScreen.get_width() / 2 + 100, mainScreen.get_height() / 2 + 50) # 下一回合点击区域
exitArea = pygame.Rect(mainScreen.get_width() / 2 - 100, mainScreen.get_height() / 2 + 100, mainScreen.get_width() / 2 + 100, mainScreen.get_height() / 2 + 100) # 下一回合点击区域
...
if ifGameGoing:
    ...
    createArmyUnit(5, 5, 5, BLACK, testGroup, gameMap)
    createArmyUnit(5, 4, 5, BLACK, testGroup, gameMap)
    nowUnit = testGroup.sprites()[unitNum]
...
while ifGameGoing:  # 游戏开始循环
    for event in pygame.event.get():
        groupLength = len(testGroup.sprites())
        if event.type == pygame.QUIT:  # 退出事件
            ifGameGoing = False
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                unitNum -= 1
                nowUnit = testGroup.sprites()[unitNum % groupLength]
        ...
        textNow = font.render("当前单位/总单位数:"+str(unitNum % groupLength + 1) + '/' + str(groupLength), True, BLACK)
